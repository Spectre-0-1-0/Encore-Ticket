...
export default function AdminScannerPage() {
  ...
  const EXPECTED_PASSCODE = process.env.NEXT_PUBLIC_ADMIN_PASSCODE || 'admin123';
  ...
  useEffect(() => {
    if (!authenticated || activeTab !== 'SCANNER') return;
  ...
  // Helper to load XLSX parser dynamically or via window / import
  ...
  // Handle CSV & Excel File Upload (.csv, .xlsx, .xls, .txt)
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (parsedRecords.length === 0) {
        setUploadErrorMsg('Could not parse any valid student records from file. Expected columns: Roll Number, Name, Email');
        return;
      }
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-4">
        <div className="glass-panel rounded-2xl p-6 sm:p-8 max-w-sm w-full shadow-2xl border border-cyan-500/20">
          ...
          <h2 className="text-xl font-bold text-center text-white mb-1">ENcore Admin Portal</h2>
          <p className="text-xs text-center text-slate-400 mb-6">Enter staff passcode to access scanner & master database upload.</p>
    ...
    <div className="max-w-xl mx-auto px-4 py-6 w-full flex-1 flex flex-col">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <a href="/" className="p-2 rounded-lg bg-slate-800 text-slate-300 hover:text-white">
            <ArrowLeft className="w-4 h-4" />
          </a>
          <div>
            <h1 className="text-lg font-bold text-white leading-tight">ENcore Admin Control</h1>
            <p className="text-[11px] text-slate-400 font-mono">GATE #1 • Security Console</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Database className="w-3.5 h-3.5" /> Upload Master
          </button>
        </div>
      </div>
      ...
      {activeTab === 'SCANNER' ? (
        ...
        /* MASTER SHEET DATABASE UPLOAD TAB */
        <div className="space-y-6">
          <div className="glass-panel rounded-2xl p-6 border border-cyan-500/20 shadow-xl space-y-5">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
                <h2 className="text-base font-bold text-white flex items-center gap-2">
                  <FileSpreadsheet className="w-5 h-5 text-cyan-400" /> Upload Master Student Sheet
                </h2>
                <p className="text-xs text-slate-400">
                  Upload CSV or Excel file (.xlsx, .xls) or paste student records to update the master database.
                </p>
              </div>
              <span className="px-3 py-1 rounded-full bg-cyan-950 border border-cyan-800 text-cyan-300 text-xs font-mono font-bold">
                {masterRecords.length} Active Records
              </span>
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-300 mb-2">
                Option 1: Choose CSV or Excel File (.csv, .xlsx, .xls)
              </label>
              <input
                type="file"
                accept=".csv,.xlsx,.xls,.txt"
                onChange={handleFileUpload}
                className="w-full text-xs text-slate-400 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-cyan-950 file:text-cyan-400 hover:file:bg-cyan-900 border border-slate-800 rounded-xl p-1 bg-slate-900 cursor-pointer"
              />
              <p className="text-[11px] text-slate-400 mt-1">Supports Excel (.xlsx, .xls) & CSV files (Columns: Roll Number, Name, Email)</p>
            </div>
            ...
            <form onSubmit={handlePasteSubmit} className="space-y-3">
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-900 border border-slate-700 text-white rounded-xl p-3 text-xs font-mono outline-none focus:border-cyan-500"
                />
              </div>

              <button
                type="submit"
                className="w-full py-3 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-bold text-xs shadow-lg shadow-cyan-600/30 transition flex items-center justify-center gap-2"
              >
          ...
          <div className="glass-panel rounded-2xl p-5 border border-slate-800">
            <h3 className="text-xs font-bold text-white uppercase tracking-wider mb-3 flex items-center gap-1.5">
              <Database className="w-4 h-4 text-cyan-400" /> Active Student Master List ({masterRecords.length})
            </h3>
